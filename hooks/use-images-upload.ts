...
export function useImagesUpload() {
  ...
  const upload = useCallback(
    async (file: File, type: 'avatar' | 'banner', maxSize: number) => {
  ...
  const uploadUserAvatar = useCallback((file: File) => upload(file, 'avatar', MAX_AVATAR_SIZE), [upload])
  const uploadUserBanner = useCallback((file: File) => upload(file, 'banner', MAX_BANNER_SIZE), [upload])
