...
export default function Footer({ variant = "primary" }: FooterProps) {
  return (
    <footer className="bg-(--color-container) border-t border-(--color-border) text-(--color-text) text-[15px] px-6 sm:px-10 py-10 text-center">
      {variant === "primary" && (
        <section className="text-(--color-text-footer) text-[0.95rem] leading-relaxed max-w-[1000px] mx-auto mb-10 px-2 sm:px-0">
          <p className="mb-5">
            Looking for a{" "}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 max-w-[1400px] mx-auto mb-8 text-center md:text-left">
        <div className="text-2xl font-bold font-[Montserrat] text-(--color-text)">
          FairPlay
        </div>
      ...
      <p className="border-t border-white/10 text-(--color-text-bold) text-sm pt-5">
